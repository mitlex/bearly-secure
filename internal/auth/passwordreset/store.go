...
func hashToken(value string) string {
	hashDigest := sha256.Sum256([]byte(value))       // hash the string (returns 32 raw unprintable bytes)
	hashedToken := hex.EncodeToString(hashDigest[:]) // encode the hash output into hexidecimal string (returns 64 printable hex chars (0-9, a-f))
	return hashedToken
}
